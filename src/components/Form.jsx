...
const Form = ({ payload, isOpen, action, text = 'actualizar', type }) => {
  ...
  const handleOnSubmit = async (e) => {
    e.preventDefault();
    if (!form.img) action({ ...form, img: 'https://www.freeiconspng.com/thumbs/no-image-icon/no-image-icon-6.png' });
    else action(form);
    isOpen(false);
  };
    ...
    return (
      <>
        <form security='off' autoComplete='off' action='https://formspree.io/f/xbjqbgko' method='POST'>
          ...
          <div className='form-floating'>
            <input className='form-control' type='email' value={form.email} autoFocus name='email' required onChange={onChange} />
            <label htmlFor='floatingInputValue'>Email</label>
          </div>
          <div className='form-check'>
            <input className='form-check-input' type='checkbox' value='' id='flexCheckDefault' required />
            <label className='form-check-label' htmlFor='flexCheckDefault'>
              Al hacer clic en "Enviar", aceptas nuestras Condiciones, la Política de datos y la Política de cookies.Es posible que te enviemos notificaciones
